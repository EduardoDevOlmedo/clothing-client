...
import {
  Container, Paper, Stack, TextField, Button, Typography, Select, MenuItem,
  FormControl, InputLabel, Alert, Snackbar,
} from '@mui/material';
import api from '../api.js';
import Nav from '../components/Nav.jsx';

const TIPOS = ['blusa', 'falda', 'botas', 'pantalon', 'vestido', 'chaqueta', 'accesorio', 'otro'];
  ...
  return (
    <>
      ...
      <Container maxWidth="sm" sx={{ py: 4 }}>
        <Paper sx={{ p: 3 }}>
          <Typography variant="h6" sx={{ mb: 2 }}>Nueva prenda</Typography>
          <form onSubmit={submit}>
            <Stack spacing={2}>
              ...
              <FormControl required>
                ...
                >
                  {ESTADOS_FISICO.map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
                </Select>
              </FormControl>

              <Stack direction="row" spacing={2} justifyContent="flex-end">
                <Button onClick={() => navigate('/')}>Cancelar</Button>
                <Button type="submit" variant="contained" disabled={saving || !form.drop}>
